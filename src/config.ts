...
export async function initConfig() {
  ...
  if (!fs.existsSync(CONFIG_PATH)) {
    ...
    console.log(
      "Edit it to set forge=bitbucket|github and the matching credentials.",
    );
  } else {
...
export function loadConfig(): Config {
  ...
  if (cfg.forge === "bitbucket" && !cfg.bitbucket)
    throw new Error("forge=bitbucket but no bitbucket config");
  if (cfg.forge === "github" && !cfg.github)
    throw new Error("forge=github but no github config");
  return cfg;
}
